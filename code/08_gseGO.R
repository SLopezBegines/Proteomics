...
KEGG_input_dataframes <- lapply(comparisons, function(comp) {
  ...
  df_UP <- data.frame(name = data_results %>% dplyr::filter(get(paste0(comp, "_diffexpressed")) == "UP") %>% pull(name),
  ...
  df_DN <- data.frame(name = data_results %>% dplyr::filter(get(paste0(comp, "_diffexpressed")) == "DOWN") %>% pull(name),

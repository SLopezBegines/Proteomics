...
assign(paste0("results_list_enrichGO_", ont), names(paste0("results_list_enrichGO_", ont)))
...
for (i in seq_along(barplot_results)) {
  if (length(barplot_results[[1]]$data$Count) == 0) {
    ...
    ggsave(paste0(filename,pdf_extension), p, width = 8, height = 12, units = "in")  # Vectorial format
    ggsave(paste0(filename,tiff_extension), p, width = 8, height = 12, units = "in")  # Tiff format
  }
}
...
for (i in seq_along(dotplot_results)) {
  if (length(dotplot_results[[1]]$data$Count) == 0) {
    cat("Plot not generated for element ", i, ".\n")
  } else {
    # Extract the desired part of the name
    plot_name <- gsub("^names_", "", names(results_list_GO)[i])  # Remove "names_"

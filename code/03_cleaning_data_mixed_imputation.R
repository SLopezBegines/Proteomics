...
#Remove "/" in output_path
  ...
  if (!is.null(output_path)) {
    pdf_file <- file.path(output_path, "figures", paste0(sprintf("%02d", image_number),"_",file_prefix, ".pdf"))
    tiff_file <- file.path(output_path, "figures", paste0(sprintf("%02d", image_number),"_",file_prefix, ".tif"))
...
proteins_MNAR <- get_df_long(data_norm) %>%
  ...
  summarize(frac_NA   = sum(is.na(intensity)) / n(),        # fraction of missing
...
compare_sd_imputations <- function(data_norm, impute_list, output_path) {
  ...
  write.xlsx(
    ...
    file = file.path(output_path, "tables", paste0("fraction_NA_",fraction_NA,
  ...
  base1 <- file.path(output_path, "figures",

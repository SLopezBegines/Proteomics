...
significant_adjusted_data <- readxl::read_xlsx(path = paste0(output_path, "tables/significant_adjusted_data.xlsx"))
significant_pval_data <- readxl::read_xlsx(path = paste0(output_path, "tables/significant_pval_data.xlsx"))
significant_qval_data <- readxl::read_xlsx(path = paste0(output_path, "tables/significant_qval_data.xlsx"))
...
pca_matrix <- data_results[, c("significant", "significance_qval", LFQ_columns)]
...
pca_loadings <- autoplot(pca_res,
  data = data_results, colour = "significant", shape = TRUE,
...
# loadings by p_value
pca_loadings <- autoplot(pca_res,
  data = data_results, colour = "significance_qval", shape = TRUE,
  label = FALSE, label.size = 3,
  loadings = FALSE, loadings.colour = "blue",
  loadings.label = TRUE, loadings.label.size = 3, frame = FALSE, frame.type = "norm"
)
...
for (i in 1:length(comparisons)) {
  diff_ <- paste(comparisons[i], "diffexpressed_qval", sep = "_")
  ratio <- paste(comparisons[i], "ratio", sep = "_")
  qval <- paste(comparisons[i], "q.val", sep = "_")
  label <- paste(comparisons[i], "dif_label_qval", sep = "_")


  vulcanogg <- suppressWarnings(
    ggplot2::ggplot(
      data = data_results,
      aes(
        x = .data[[ratio]],
      ...
      labs(
        x = (paste("Fold-Change ", comparisons[i])),
...
heatmap_adj <- pheatmap::pheatmap(heatmap_matrix_padj,
  cutree_rows = 4,
  cluster_cols = FALSE,
  scale = "none",
...
heatmap_df <- data_results %>%
  dplyr::filter(significance_qval == TRUE) %>%
  dplyr::select(name, ends_with("_ratio")) %>%
  # dplyr::arrange(desc(!!!syms(selected_columns))) %>%
  dplyr::select(all_of(specific_order))
...
# Plot heatmap with specific order of cluster names on x-axis
heatmap_int <- pheatmap::pheatmap(heatmap_matrix_exclude_int_2,
  cutree_rows = 4, row_names = TRUE,
  cluster_cols = FALSE, scale = "none",
  # annotation_col = label,
  # annotation_colors = list(Gene_Present = c("No" = "white", "Yes" = "black")),
  annotation_legend = TRUE,
